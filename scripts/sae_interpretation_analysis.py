#!/usr/bin/env python3
"""
SAE Feature Interpretation Analysis Script

This script loads existing SAE attribution analysis results and generates
comprehensive interpretability analysis including:

1. Feature specificity scores and plots
2. Attribution leakage analysis
3. Feature-attribution heatmaps
4. Interactive feature exploration
5. Detailed interpretation reports

Usage:
    python scripts/sae_interpretation_analysis.py --input-dir results/SMALL_test_sae_analysis --output-dir results/sae_interpretation
"""
...
class SAEFeatureInterpreter:
    ...
    def load_data(self):
        """Load analysis results, SAE features, and metadata."""
        # Load analysis results
        with open(self.results_path / 'analysis_results.json', 'r') as f:
            self.results = json.load(f)

        # Load SAE features
        self.sae_features = np.load(self.results_path / 'sae_features.npy')
        self.n_cells, self.n_features = self.sae_features.shape

        # Load metadata
        self.metadata = pd.read_csv(self.results_path / 'cell_metadata.csv', index_col=0)

        # Extract significance and effect size matrices
    ...
    def _extract_significance_matrices(self) -> dict:
    ...
    def _extract_effect_size_matrices(self) -> dict:
            ...
            for j, attr_name in enumerate(attr_names):
                ...
                for result in attr_results:
                    if 'feature_idx' in result and 'effect_size' in result:
                        ...
                        if 0 <= feature_idx < self.n_features:
                            matrix[feature_idx, j] = abs(effect_size)
        ...
        for attr_type, data in self.significance_matrices.items():
            ...
            for attr_idx, attr_name in enumerate(attr_names):
                ...
                if n_features_affected > 0:
                    ...
                    leakage_score = n_features_affected / self.n_features
    ...
    def plot_feature_specificity_distribution(self, output_dir: str):
        ...
        axes[1, 1].set_yticklabels([f"F{idx}: {attr[:20]}" for idx, attr in
    ...
    def plot_attribution_leakage_analysis(self, output_dir: str):
        ...
        color_map = {'binary': 0, 'categorical': 1, 'continuous': 2}
        colors = [color_map.get(t, 0) for t in leakage_df['attr_type']]
        ...
        axes[1, 1].set_yticks(y_pos)
        axes[1, 1].set_yticklabels([f"{attr[:15]} ({n})" for attr, n in
    ...
    def create_feature_landscape_analysis(self, output_dir: str):
        """Create PCA-based feature landscape analysis."""
        # Compute feature embeddings using PCA
        ...
        # PCA colored by specificity
        scatter = axes[0, 0].scatter(feature_info['pca_x'], feature_info['pca_y'],
                                   c=feature_info['specificity_score'],
                                   s=feature_info['activation_mean']*100,
                                   alpha=0.6, cmap='viridis')
        axes[0, 0].set_xlabel(f'PC1 ({pca.explained_variance_ratio_[0]:.1%} variance)')
        axes[0, 0].set_ylabel(f'PC2 ({pca.explained_variance_ratio_[1]:.1%} variance)')
        axes[0, 0].set_title('Feature Landscape (colored by specificity)')
        plt.colorbar(scatter, ax=axes[0, 0], label='Specificity Score')

        # PCA colored by sparsity
        scatter2 = axes[0, 1].scatter(feature_info['pca_x'], feature_info['pca_y'],
                                    c=feature_info['sparsity'],
                                    s=50, alpha=0.6, cmap='plasma')
        axes[0, 1].set_xlabel(f'PC1 ({pca.explained_variance_ratio_[0]:.1%} variance)')
        axes[0, 1].set_ylabel(f'PC2 ({pca.explained_variance_ratio_[1]:.1%} variance)')
        axes[0, 1].set_title('Feature Landscape (colored by sparsity)')
        plt.colorbar(scatter2, ax=axes[0, 1], label='Sparsity')

        # Specificity vs sparsity
        axes[1, 0].scatter(feature_info['sparsity'], feature_info['specificity_score'], alpha=0.6)
        axes[1, 0].set_xlabel('Feature Sparsity')
        axes[1, 0].set_ylabel('Specificity Score')
        axes[1, 0].set_title('Specificity vs Sparsity')

        # Activation distribution
        axes[1, 1].hist(feature_info['activation_mean'], bins=50, alpha=0.7, edgecolor='black')
        axes[1, 1].set_xlabel('Mean Activation')
        axes[1, 1].set_ylabel('Number of Features')
        axes[1, 1].set_title('Distribution of Feature Activations')

        plt.tight_layout()
        plt.savefig(f"{output_dir}/feature_landscape_analysis.png", dpi=300, bbox_inches='tight')
        plt.close()

        # Save feature landscape data
        feature_info.to_csv(f"{output_dir}/feature_landscape_data.csv", index=False)
        logger.info(f"Feature landscape analysis saved to {output_dir}")
    ...
    def run_full_analysis(self, output_dir: str):
        ...
        self.create_feature_landscape_analysis(output_dir)
        report = self.generate_interpretation_report(output_dir)
...
def main():
    """Main interpretation analysis."""
    parser = argparse.ArgumentParser(description="SAE Feature Interpretation Analysis")
    parser.add_argument("--input-dir", type=str, required=True,
                       help="Directory containing SAE analysis results")
    parser.add_argument("--output-dir", type=str, default="results/sae_interpretation",
                       help="Output directory for interpretation analysis")
    ...
    required_files = ['analysis_results.json', 'sae_features.npy', 'cell_metadata.csv']
    ...
    try:
        interpreter = SAEFeatureInterpreter(args.input_dir)
        report = interpreter.run_full_analysis(args.output_dir)

        print("\n" + "="*60)
        print("SAE FEATURE INTERPRETATION ANALYSIS COMPLETE")
        print("="*60)
        print(f"\nResults saved to: {args.output_dir}")
        print("Key files generated:")
        print("  - feature_specificity_analysis.png")
        print("  - attribution_leakage_analysis.png")
        print("  - feature_attribution_heatmap.png")
        print("  - feature_landscape_analysis.png")
        ...
        logger.error(f"Interpretation analysis failed: {e}")
        raise
